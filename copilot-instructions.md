...
- Key files & folders to inspect first
  ...
  - `R/` — implementation. Start with `ggseg.R`, `geom-brain.R`,
    ...
    flows.
  - `data/` — prebuilt atlas `.rda` datasets (e.g. `dk.rda`,
    `aseg.rda`).
...
- Developer workflows (commands/examples)
  - Update docs after changing roxygen:
  ...
  - Run unit tests:
    ...
    (testthat 3rd edition; tests run in parallel via
...
- Integration points & native deps
  - `sf` requires GDAL/GEOS/PROJ on the system — CI and local dev must
    have these installed. `SystemRequirements` in `DESCRIPTION` lists
    minima (GDAL \>= 2.0.1, GEOS \>= 3.4.0, PROJ \>= 4.8.0).
...
- Linting, formatting, and CI notes
  - Project uses `lintr`; add `.lintr` to ignore large auto-generated
    folders like `data-raw/` if not already ignored:
    `exclude: "data-raw/"`.
  - PDF manual requires LaTeX packages (e.g. `inconsolata`); expect PDF
    creation to fail on systems missing TeX packages — CI may differ
    from local systems.
- Example change pattern (edit + verify)
  ...
  4.  Run
      [`devtools::check()`](https://devtools.r-lib.org/reference/check.html)
      and fix warnings/notes before PR.
